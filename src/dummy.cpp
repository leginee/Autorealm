...
// We also use this file to act as the primary pointer page for doxyegn.
// The reason for this is simple: This reduces build dependencies for
 ...
 *
 * We've put major effort into making the framework for AutoRealm_wx as
 * thorough as possible. The results show, I think.
 * 
 * At this time, the program compiles, runs, and displays a window.
 * 
 * The framework is in place to show help files, load files, and save
 * files. Basic object information is defined. Unit Testing works with
 * our data to make sure we have as much as possible tested before
 * we release.
 * 
 ...
 *                 generate the ui)
 *         - portsrc (this directory holds failed efforts to produce a
 *         				line by line port, which will be useful in solving
 *         				some problems in the rewrite)
 * 
 * @section standards Coding Rules / Standards
 * -# Class Names: EachWordIsUpperCased
 * -# Method Names: firstWordLowerCased
 * -# Member variables of classes: prefixed with m_
 * -# Parameters coming into a method/function: prefixed with p_
 ...
 * -# All code needs to have liberal TRACE statements (every couple of lines
 ...
 *
 * We were doing a straight port of the code. This was leading to code that
 * was, to put it politely, crap. Bugs were being left in, code was untestable,
 * and we were hurting for ways to do things that should have been easy to do.
 * 
 * We scrapped the code, and began anew. We are now working on a program that
 * will read all old AutoRealm files, write a newer format, and make things
 * look better and run faster.
 * 
 * We hope you'll agree that the results are worth it.
 */
/**
 ...
 *    your path!
 * -# If you need it, download <a href="http://ctags.sourceforge.net/">ctags</a>
 *    and install it. Make sure to put ctags into a directory in your path!
 *
 ...
 *    into &quot;C:\\MinGW&quot;
 * -# Download <a href="http://prdownloads.sf.net/mingw/msys-libtool-1.5.tar.bz2?download">the
 *    updated version of libtool</a> to the folder C:\\Temp, and then do the
 *    following commands in an MSyS shell:
 *      - cd /
 *      - tar -xjf /c/temp/msys-libtool-1.5.tar.bz2
 ...
 *    TRACE. If you don't care to see the output, then don't
 *    worry about installing this tool.
 * -# Download <a href="http://cppunit.sourceforge.net/">CppUnit</a> and install it
 *  	- cd /c/temp
 ...
 *      - mkdir build-dbg
 *      - cd build-dbg
 *      - ../configure --enable-unicode --enable-debug --with-expat --prefix=c:/mingw/local
 *      - make && make install
 *      - cd src/expat
 *      - rm libtool
 ...
 *      - mkdir build-prd
 *      - cd build-prd
 *      - ../configure --enable-unicode --disable-debug --with-expat --prefix=c:/mingw/local
 *      - make && make install
 *      - cd src/expat
 *      - rm libtool
 *      - ln -s /bin/libtool libtool
 ...
 * build them.  
 *
 * -# Using your package manager, install both expat, and the header files for it.
 * -# Using your package manager, install cppunit
 * -# Download <a href="http://www.wxwidgets.org/">wxWidgets</a>.
 *    @note Make sure to download release 2.6.0, and also make sure to download a
 *    copy of the documentation in your chosen format (chm, html, pdf, etc).
 *    Save them in the following locations (the steps below assume you do
 ...
 * information in @ref developing to begin developing now.
 *
 * @section environment_mac Configuring For Mac OS X Development
 *
 * @note - You will build two versions of wxWidgets by following these steps.
 * This is deliberate, as wxWidgets provides some facilities only available
 * when debugging is turned on. It is useful to have both versions of the
 * library available, so these instructions make sure you build them.
 * 
 * @note - You will need to have the Apple DevTools (aka XCode) installed
 * in order to build these tools.  As with most development tasks you will
 * need to be logged in as administrator and have the root password.
 *
 * @note - It is assumed that all downloaded files are downloaded to your home
 * directory.  This is ~ from the command line.  If you use Safari to download the
 * app it will unzip it for you in which case, leave off the z from the tar commands 
 * and the .gz from the files names below.
 * 
 * -# Download and install the applications from @ref environment_common
 *      - There are other options for the mac including excellent "macized" versions of:
 *      - <a href="http://www.pixelglow.com/graphviz/">GraphViz</a>
 *      - <a href="ftp://ftp.stack.nl/pub/users/dimitri/Doxygen-1.4.4.dmg">Doxygen</a>
 * -# Set up your path variables to sidestep the existing wxMac2.5 installation.
 *    @note You can add this to your .profile to make it permanent.
 *
 ...
 *      - make
 *      - sudo make install (root password needed)
 * -# Download <a href="http://www.wxwidgets.org/">wxWidgets</a> and install it.
 *    @note Make sure to download release 2.6.1, and also make sure to download a
 *    copy of the documentation in your chosen format (chm, html, pdf, etc).
 * 
 *      - cd ~
 *      - tar xzf wxMac-2.6.1.tar.gz
 *      - tar xzf wxWidgets-2.6.1-(your format here).tar.gz
 *      - cd wxMac-2.6.1
 *      - mkdir build-dbg build-prd
 *      - cd build-dbg
 *      - ../configure --enable-unicode --enable-debug --with-expat --disable-shared --enable-static --prefix=/usr/local
 *      - make
 *      - sudo make install (root password needed)
 *      - cd ../build-prd
 *      - ../configure --enable-unicode --disable-debug --with-expat --disable-shared --enable-static --prefix=/usr/local
 *      - make
 *      - sudo make install (root password needed)
 * -# Install expat from the wxMac source.
 *      - cd ~/wxMac-2.6.1/src/expat
 *      - ./configure --enable-unicode --disable-shared --enable-static --prefix=/usr/local
 *      - make
 *      - sudo make install (root password needed)
 *
 * You are done preparing your system for building AutoREALM, and may use the
 ...
 *
 * - svn co https://svn.sourceforge.net/root/autorealm
 *
 ...
 *
 * @section dev_gen_objtypes Adding Object Types
 *
 * This is actually a fairly complex procedure. Being do-able does not make
 * it easy.
 *
 * To summarize the steps
 * - Define the object, and test cases
 * - Update the appropriate copy/compare functions. For instance, when
 *   adding PushpinCollection, it was necessary to modify
 *   ARDocument::compare() and ARDocument::copy
 * - Modify ARDocumentTest to add the new object type, and verify that it
 *   adds correctly, and that all test cases still pass
 * - Modify ObjectBuilderTest to load the new object. See ObjectBuilder for
 *   information on how to add SubBuilder's and the like.
 * - Modify ObjectBuilder to load the new object type from file
 * - Modify ObjectWriterTest to verify the object type is written correctly
 *   to file
 * - Modify ObjectWriter to save the new object type
 *
 * While more <em>could</em> be written on these steps, this is all that
 * will be written at this time.
 */
/**
