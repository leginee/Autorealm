...
TRACEFLAG(wxT("mmMultiButton"));
namespace {
	#include "down_btn.xpm"
	#include "disable.xpm"
	wxBitmap gDownBM;
	wxBitmap gDisableBM;

	wxColor mmDARK_GREY(100,100,100);
}
                           ...
                           const long int style)
{
	...
	wxBitmap DownBM(down_btn_xpm);
	wxBitmap DisableBM(wxDISABLE_BUTTON_BITMAP);
	gDownBM = DownBM;
	gDisableBM = DisableBM;
...
void mmMultiButton::FindAndSetSize()
{
    ...
    { // Refresh and set size of button.
        ...
        if (mLabelStr != wxT("") && mDefaultBitmap.Ok())
            h_extra += mMarginSize;
        total_width  += w_extra;
...
// Redraw icons
{
	...
	wxASSERT(mDefaultBitmap.Ok() == true);
    wxBitmap bm = mDefaultBitmap; // Default bitmap
    if (((mStyle & mmMB_FOCUS) && mHasFocus) and mFocusBitmap.Ok())
        bm = mFocusBitmap; // Focused bitmap
    if (((mStyle & mmMB_SELECT) && mIsSelected) and mSelectedBitmap.Ok())
        bm = mSelectedBitmap; // Selected bitmap
    if (((mStyle & mmMB_TOGGLE) && mIsToggleDown) and mToggleBitmap.Ok())
...
IMPLEMENT_DYNAMIC_CLASS(mmMultiButtonXmlHandler, wxXmlResourceHandler)

mmMultiButtonXmlHandler::mmMultiButtonXmlHandler() : wxXmlResourceHandler() {
	TRACER(wxT("mmMultiButtonXmlHandler"));
	XRC_ADD_STYLE(mmMB_FOCUS);
	XRC_ADD_STYLE(mmMB_SELECT);
	XRC_ADD_STYLE(mmMB_TOGGLE);
	XRC_ADD_STYLE(mmMB_DROPDOWN);
	XRC_ADD_STYLE(mmMB_WHOLEDROPDOWN);
	XRC_ADD_STYLE(mmMB_STATIC);
	XRC_ADD_STYLE(mmMB_NO_AUTOSIZE);
	XRC_ADD_STYLE(mmMB_AUTODRAW);
	AddWindowStyles();
}

bool mmMultiButtonXmlHandler::CanHandle(wxXmlNode* node) {
	TRACER(wxT("CanHandle"));
	return(IsOfClass(node, wxT("mmMultiButton")));
}

wxObject* mmMultiButtonXmlHandler::DoCreateResource() {
	TRACER(wxT("DoCreateResource"));
	XRC_MAKE_INSTANCE(button, mmMultiButton);

	wxBitmap loader(wxDISABLE_BUTTON_BITMAP);
	wxString label(GetText(wxT("label")));
	button->Create(m_parentAsWindow, GetID(), label,
				   loader, GetPosition(), GetSize(), GetStyle());
	SetupWindow(button);
	if (!GetParamValue(wxT("bitmap")).IsEmpty()) {
		loader = GetBitmap(wxT("bitmap"));
		button->SetDefaultBitmap(loader);
	}
	if (!GetParamValue(wxT("selectedbitmap")).IsEmpty()) {
		button->SetSelectedBitmap(GetBitmap(wxT("selectedbitmap")));
	} else {
		button->SetSelectedBitmap(loader);
	}
	if (!GetParamValue(wxT("focusbitmap")).IsEmpty()) {
		button->SetFocusBitmap(GetBitmap(wxT("focusbitmap")));
	} else {
		button->SetFocusBitmap(loader);
	}
	if (!GetParamValue(wxT("togglebitmap")).IsEmpty()) {
		button->SetToggleBitmap(GetBitmap(wxT("togglebitmap")));               
	} else {
		button->SetToggleBitmap(loader);
	}
	button->SetToggleDown(GetBool(wxT("toggled")));
	button->SetName(GetName());
	return(button);
}

namespace {
	class InitmmMultiButtonXmlHandler {
		public:
			InitmmMultiButtonXmlHandler() {
				wxXmlResource::Get()->AddHandler(new mmMultiButtonXmlHandler);
			}
	};
	InitmmMultiButtonXmlHandler mmxml;
}
