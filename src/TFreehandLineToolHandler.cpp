...
#include "globals.h"
#include "TFreehandLineToolHandler.h"

#include "Map.xpm"

static wxString className=wxT("TFreehandLineToolHandler");

TFreehandLineToolHandler::TFreehandLineToolHandler(wxDC* canvas) : TNormalPolyLineToolHandler(canvas) {
	wxLogTrace(className, wxT("TFreehandLineToolHandler(wxDC* canvas): beginning"));
	wxLogTrace(className, wxT("TFreehandLineToolHandler(wxDC* canvas): ending"));
}

void TFreehandLineToolHandler::Cancel() {
	wxLogTrace(className, wxT("Cancel(): beginning"));
	wxLogTrace(className, wxT("Cancel(): ending"));
}

bool TFreehandLineToolHandler::MouseDown() {
	wxLogTrace(className, wxT("MouseDown(): beginning"));
	wxLogTrace(className, wxT("MouseDown(): ending"));
}

void TFreehandLineToolHandler::MouseMove() {
	wxLogTrace(className, wxT("MouseMove(): beginning"));
	wxLogTrace(className, wxT("MouseMove(): ending"));
}

void TFreehandLineToolHandler::MouseUp() {
	wxLogTrace(className, wxT("MouseUp(): beginning"));
	wxLogTrace(className, wxT("MouseUp(): ending"));
}

void TFreehandLineToolHandler::CreateClosedFigure() {
	wxLogTrace(className, wxT("CreateClosedFigure(): beginning"));
	wxLogTrace(className, wxT("CreateClosedFigure(): ending"));
}

void TFreehandLineToolHandler::CreateOpenFigure() {
	wxLogTrace(className, wxT("CreateOpenFigure(): beginning"));
	wxLogTrace(className, wxT("CreateOpenFigure(): ending"));
}

void TFreehandLineToolHandler::Backspace() {
	wxLogTrace(className, wxT("Backspace(): beginning"));
	wxLogTrace(className, wxT("Backspace(): ending"));
}

void TFreehandLineToolHandler::Done() {
